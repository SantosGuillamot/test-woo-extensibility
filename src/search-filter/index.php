<?php
/**
 * Hook into the Woo Product Filter block to convert it into an instant search filter.
 */

// Filter the query loop to understand the `filter_search` URL parameter.
function test_extensibility_add_url_filtering( $query, $block ) {
...
add_filter( 'query_loop_block_query_vars', 'test_extensibility_add_url_filtering', 20, 2 );

// Add directives to the Product Search block to filter by search terms.
function test_extensibility_add_directives_to_product_search( $block_content, $block ) {
	wp_enqueue_script_module( 'woocommerce-interactivity-scripts' );

	if ( isset( $block['attrs']['namespace'] ) && 'woocommerce/product-search' !== $block['attrs']['namespace'] ) {
...
add_filter( 'render_block_core/search', 'test_extensibility_add_directives_to_product_search', 10, 2 );